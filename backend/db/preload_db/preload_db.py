...
async def preload_db():
    async with async_session_factory() as session:
        ...
        await GasVolumeCalcTypeDao(session=session).bulk_upsert(
        ...
        await EditTypeDao(session=session).bulk_upsert(
        ...
        for i in range(0, len(instance_list), batch_size):
            ...
            await SysTypeDao(session=session).bulk_upsert(batch, SYS_TYPE_CONSTRAINT)
